    ...
    def run(self):
        try:
            ...
            with paramiko.SSHClient() as ssh_client:
                ...
                self.update_signal.emit("Listing of Devices from the Active ports...")
                ...
                with paramiko.SSHClient() as ssh_client:
                    ...
                    if hostname in self.device_config_map:
                        ...
                        for i in range(0, len(device_config), batch_size):
                            ...
                            self.update_signal.emit(f"Sending batch #{batch_number} of commands to port {port} from file {config_file}")
                            ssh_shell.send(command_batch)
                            self.update_signal.emit(f"Waiting for response of {batch_size} commands from {hostname} for batch #{batch_number} from file {config_file}")
                            ssh_shell.send("\r")
                            ...
                            self.update_signal.emit(f"Batch #{batch_number} processed, response: {response}")
                        ...
                        #     ssh_shell.send(command_batch)
                        #     self.update_signal.emit(f"Waiting of responce of 100 commands from {hostname} from file {config_file}")
                        #     ssh_shell.send("\r")
                        #     time.sleep(2)  # Wait for the commands to be processed
