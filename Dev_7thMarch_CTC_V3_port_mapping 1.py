...
class SaveLocationDialog(QMainWindow):  #This class used for GUI design and button handler
    ...
    def initUI(self):
        ...
        # mainLayout.addWidget(self.clearButton, 3, 4, 1, 2)
    ...
    #     # Reset buttons
    #     self.check_button.setEnabled(True)  # Enable the check button
    #     self.connect_button.setEnabled(False)  # Disable the connect button
        # Add more button reset logic if needed

        # Clear data
        #self.server_ip_input.setText('')  # Clear server IP input
        #self.username_input.setText('')  # Clear username input
        #self.password_input.setText('')  # Clear password input
        # self.Switch_username.setText('')  # Clear switch username input
        # self.Switch_password_input.setText('')  # Clear switch password input
        # self.clearTable()  # Clear devices table
        # self.logTextBox.clear()  # Clear log text box
        
        # Clear log text box except for the first line
        # if self.logTextBox.toPlainText():
        #     lines = self.logTextBox.toPlainText().split('\n')
        #     first_line = lines[0]
        #     rest_of_lines = '\n'.join(lines[1:])
        #     self.logTextBox.setPlainText(first_line + '\n' + rest_of_lines)
        # else:
        #     self.logTextBox.clear()
        # Add more variable clearing logic for other variables if needed
       # print("Buttons reset and data cleared")

    def clearTable(self):
        self.devices_table.setRowCount(0)  # Clear table rows
        self.devices_table.clearContents()  # Clear table contents

        # Enable browse and push config buttons if table has values
        if self.devices_table.rowCount() > 0:
            self.browseButton.setEnabled(True)
            self.pushConfigButton.setEnabled(True)
    ...
    def update_devices_table(self, data):
        ...
        # Enable the browse_button and push_config_button if devices_table has data
        if self.devices_table.rowCount() > 0:
            self.browse_button.setEnabled(True)
            self.push_config_button.setEnabled(True)
        else:
