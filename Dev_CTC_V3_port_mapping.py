...
class SaveLocationDialog(QMainWindow):  #This class used for GUI design and button handler
    ...
    def initUI(self):
        ...
        mainLayout.addWidget(self.check_button, 0, 2)    # Row 0, Column 2
        ...
        mainLayout.addWidget(self.get_config_button, 1, 2)  # Row 1, Column 2
        mainLayout.addWidget(self.get_reload_button, 1, 3)  # Row 1, Column 3
        ...
        mainLayout.addWidget(devicesConsoleWidget, 3, 0, 1, 5)  # Span 5 columns
    ...
    def pushConfig(self):
        # QMessageBox.information(self, "Select Log Directory", "Please CLICK OK to select a path to save the logs.")

        # # Prompt the user to select a log directory
        # selected_log_dir_path = QFileDialog.getExistingDirectory(self, "Select Log Directory", "")
        # if not selected_log_dir_path:
        #     # User canceled the selection or did not select a directory
        #     QMessageBox.information(self, "Log Directory", "Log directory selection is required to push the config.")
...
class MainProcessThread(QThread):     #this class is being use for Connect to Server and get Port Number and Serial Number
        ...
        try:
            ...
            with paramiko.SSHClient() as ssh_client:
                ...
                for port_info in active_ports:
                    ...
                    device_username = "wwt"
                    device_password = "T_Mobile123"
                    port_specific_username = f"{self.username}:port{port}"
                    print("port_specific_username: ", port_specific_username)
                    ...
                    end_time = time.time() + 5  # 10 second timeout for prompt detection
                    prompt_detected = False

                    while time.time() < end_time:
                        ssh_shell.send("\r")  # Send a carriage return to provoke a prompt
                        time.sleep(1)
                        if ssh_shell.recv_ready():
                            output = ssh_shell.recv(9999).decode('utf-8')
                            if output.strip().endswith('#'):  # Assuming # is your prompt
                                prompt_detected = True
                                break  # Prompt detected, break out of the loop

                    if not prompt_detected:
                        # Prompt was not detected, emit an update and skip to the next port
                        self.update_signal.emit(f"Prompt not detected for port {port}. Skipping to next port.")
                        ssh_client.close()
                        continue
                    ...
                    if "Username:" in output or "login:" in output:
                        ...
                        time.sleep(2)
                        output = self.debug_output(ssh_shell)  # Capture output after login
                    else:
                        self.update_signal.emit("No Username/login prompt, proceeding with alternative actions")
                        # You might want to send some initial commands here if needed
    ...
    #             self.update_signal.emit('Active ports: ' + ', '.join(active_ports))
    #             #hostname_pattern = r"\n(\S+)>"  # This pattern is already suitable for matching hostnames with or without a space before the prompt character.
    #             #hostname_pattern = r"(^|\n)(\S+)\s*[>#]"
    ...
    #                 # hostname_match = re.search(hostname_pattern, prompt_output)
    #                 # if hostname_match:
    #                 #     #hostname = hostname_match.group(2).strip()
    #                 #     hostname = hostname_match.group(1).strip()  # Strip in case of any whitespace
    #                 #     self.update_signal.emit(f"Hostname: {hostname} for Port: {port}")
    #                 # else:
    #                 #     self.update_signal.emit(f"Hostname not found for Port: {port}")
...
class ReloadDeviceThread(QThread):    #this class is being used for relod the device.
    ...
    def __init__(self, server_ip, username, password, selected_ports,parent=None):
        super().__init__(parent)
        self.server_ip = server_ip
        self.username = username
        self.password = password
        ...
        try:
            ...
            for port, hostname in self.selected_ports:  # Assuming selected_ports includes (port, hostname) tuples
                with paramiko.SSHClient() as ssh_client:
                    ...
                    if "System configuration has been modified. Save?" or "Proceed with reload? [confirm]" in response:
                        ssh_shell.send('yes\r')  # Send 'yes' to save configuration
                        time.sleep(1)
