import sys
import openpyxl
from PyQt5.QtWidgets import QApplication,QGridLayout,QMainWindow, QInputDialog,QTextEdit,QPushButton,QMessageBox , QWidget, QFileDialog, QLineEdit, QLabel
        ...
        try:
            ...
            if console_output.strip() == '#':
                ...
                for row in table_data:
                    ...
                    time.sleep(2)
                    output = ""
                    while not ssh_shell.recv_ready():  # Wait until there is data to be read
                        time.sleep(0.1)
                    while ssh_shell.recv_ready():
                        output += ssh_shell.recv(1024).decode('utf-8')

                    # Check for the 'Username:' prompt
                    username_prompt = re.search(r"Username:", output)
                    if username_prompt:
                        self.update_signal.emit("Username prompt detected")
                        ssh_shell.send(str(device_username) + "\r")
                        time.sleep(2)
                        ssh_shell.send(str(device_password) + "\r")
                        time.sleep(2)
                        ...
                        self.update_signal.emit("No Username prompt, proceeding with 'enable' command")
                        ssh_shell.send("enable\r")
                        time.sleep(1)
...
class MainProcessThread(QThread):
    ...
    def openInputDialog(self, ssh_shell, device_username, device_password):
        device_username, ok = QtWidgets.QInputDialog.getText(None, "Device Login", "Enter device username:")
        if not ok:
            self.update_signal.emit("Device username not entered.")
            return

        device_password, ok = QtWidgets.QInputDialog.getText(None, "Device Login", "Enter device password:")
        if not ok:
            self.update_signal.emit("Device password not entered.")
            return

        ssh_shell.send(device_username)
        ssh_shell.send("\r")

        while not console_output.endswith("Password: "):
            console_output += self.debug_output(ssh_shell)

        ssh_shell.send(device_password)
        ssh_shell.send("\r")
    ...
    def run(self):
        try:
            ...
            if console_output.strip() == '#':
                ...
                worksheet['E1'] = "device_password"  
                worksheet['F1'] = "enable_password"   # code change
                worksheet['G1'] = "Device Config"
                worksheet['H1'] = "Output"
                ...
                # for idx, port in enumerate(active_ports, start=2):
                #     worksheet[f'A{idx}'] = port
                ...
                for port in output:
                    ...
                    self.update_signal.emit("My output: " + console_output)
                    QTimer.singleShot(0, lambda: self.openInputDialog(ssh_shell))
                    time.sleep(2)
                    ssh_shell.send("\r")
                    time.sleep(2)  
                ...
                for idx, (match, serial_number, device_type) in enumerate(matches_with_output, start=2):
                    ...
                    if device_type:
                        worksheet[f'C{idx}'] = device_type
                # for idx, port in enumerate(output, start=2): #from line 507 to 509 only for the active port
                #     worksheet[f'A{idx}'] = port
                workbook.save(self.excel_path)
                #print("Completed")
                self.update_signal.emit("Completed")
...
class SaveLocationDialog(QMainWindow):
    ...
    def initUI(self):
        ...
        # self.additionalTextBox = QLineEdit(self)  # Changed from QTextEdit to QLineEdit
        # self.additionalTextBox.setPlaceholderText("Enter additional information here...")
        # layout.addWidget(QLabel("Additional Information:"), 7, 0)  # Adjust the position just above the Activity Console
        # layout.addWidget(self.additionalTextBox, 7, 1)  # Place it next to its label in the same row
