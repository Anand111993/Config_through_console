        ...
        try:
            ...
            for port, hostname in self.selected_ports:
                ...
                with paramiko.SSHClient() as ssh_client:
                    ...
                    #     if "Login incorrect" in output:
                    #         self.update_signal.emit(f"Incorrect username/password, Can't push for {hostname}")
                    #         continue
                        
                    #     # Wait for the hostname prompt
                    #     hostname_pattern = r"(\S+)[>#](\s*|\s+\S*)$"
                    #     #hostname_pattern = r"(\S+)[>#]([\s\S]*?)$"
                    #     compiled_pattern = re.compile(hostname_pattern)
                    #     end_time = time.time() + 10  # 15 seconds timeout
                    ...
                    #             output += received
                    #             print(f"DEBUG: Accumulated output: {output}")
                    ...
                    if hostname in self.device_config_map:
                        ...
                        batch_size = 30  # Adjust based on your device's capability
