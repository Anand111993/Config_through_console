...
class SaveLocationDialog(QMainWindow):  #This class used for GUI design and button handler
    ...
    def pushConfig(self):
        ...
        if not selected_ports:
            ...
            self.push_config_button.setEnabled(True)
            self.updateLogTextBox("No devices selected for configuration.")
            return
        ...
        print("Port batches:", self.port_batches)  
        
        # Start processing the first batch if any
        if self.port_batches:
            print("Calling processBatch...")
    ...
    def processBatch(self, ports_batch, device_config_map, save_directory):
        print(f"Processing batch: {ports_batch}") 
        self.active_threads = len(ports_batch)
        for port, hostname in ports_batch:
            print(f"Checking device: {hostname} in device_config_map...") 
            if hostname in device_config_map:
                print(f"Device {hostname} found in device_config_map.")
                config_file = device_config_map[hostname]
                thread = DeviceConfigThread(
                    server_ip=self.server_ip_input.text().strip(),
                    username=self.username_input.text().strip(),
                ...
                thread.finished.connect(self.onThreadFinished)  # Directly connect
                self.threads.append(thread)
                thread.update_signal.connect(self.updateLogTextBox)
                thread.start()
                print(f"Thread started for device: {hostname} on port {port}")  # Print to console
            else:
                print(f"Device {hostname} not found in device_config_map.")  # Debug    
    ...
    def match_config_files_to_devices(self):
        print(f"Selected config files: {self.selected_config_files}") 
...
class DeviceConfigThread(QThread):     #This class is being used for pushconfig
    update_signal = pyqtSignal(str)
    ...
    def read_shell_output(self, ssh_shell, timeout=3):
        """Reads the complete SSH output and ensures it is emitted correctly."""
        output = ""
        end_time = time.time() + timeout
        ...
        while time.time() < end_time:
            if ssh_shell.recv_ready():
                try:
                    new_data = ssh_shell.recv(4096).decode('utf-8', errors="ignore")  # Decode safely
                    output += new_data
                    self.update_signal.emit(new_data)  # Emit response in real-time
                    end_time = time.time() + timeout  # Reset timeout if data is still coming
                except Exception as e:
                    self.update_signal.emit(f"Error reading SSH output: {str(e)}")
                    break
            else:
                QThread.msleep(100)  # Avoid CPU overload, wait briefly
    ...
    def send_config_in_batches(self, ssh_shell, config_file, hostname):
        """ Sends configuration in batches for optimized processing """
        batch_size = 100  # Adjust based on device capability
        with open(config_file, 'r') as file:
            device_config = file.readlines()

        for i in range(0, len(device_config), batch_size):
            command_batch = ''.join(device_config[i:i+batch_size]) + '\n'
            self.update_signal.emit(f"Sending batch to {hostname}...")
            ssh_shell.send(command_batch)
            ssh_shell.send("\r")
            QThread.msleep(2000)  # Allow processing time

            response = self.read_shell_output(ssh_shell)
            self.update_signal.emit(f"Batch processed for {hostname}: {response}")

    def send_command_and_wait(ssh_shell, command, hostname_pattern, wait_time=90):
        """
        Sends a command to the switch and waits dynamically until execution is complete.
        """
        ...
        while time.time() - start_time < wait_time:
            if ssh_shell.recv_ready():
                ...
                # Check if the device prompt has returned, indicating execution is complete
                if re.search(hostname_pattern, complete_response, re.MULTILINE):
                    break
            else:
                QThread.msleep(500)  # Avoid aggressive polling
    ...
    def run(self):
        try:
            ...
            log = None  # Initialize log file variable
            error_log = None  # Initialize error log file variable
            ...
            for port, hostname in self.selected_ports:
                try:
                    ...
                    device_password = "T-Mobile123"
                    formatted_port = f"port{int(port):02d}"
                    self.update_signal.emit(f"Connecting to {hostname} on {formatted_port}...")
                    ...
                    if hostname in self.device_config_map:
                        ...
                        if not os.path.isfile(config_file):
                            self.update_signal.emit(f"Error: Config file not found for {hostname} - {config_file}")
                            continue

                        if os.stat(config_file).st_size == 0:
                            self.update_signal.emit(f"Error: Config file is empty for {hostname} - {config_file}")
                            continue
                        ...
                        try:
                            ...
                            self.update_signal.emit(f"Error: Cannot read config file for {hostname}: {str(e)}")
                            continue
                        ssh_shell.send("enable\r")
                        QThread.msleep(2000)
                        ssh_shell.send("Bdc4Z!KF18MNmgIg29lRW@eZkr")
                        ssh_shell.send("conf t\r")
                        QThread.msleep(2000)

                        batch_size = 100
                        total_batches = len(device_config) // batch_size + (1 if len(device_config) % batch_size else 0)

                        for batch_index in range(total_batches):
                            start = batch_index * batch_size
                            end = start + batch_size
                            command_batch = '\n'.join(device_config[start:end]) + '\n'

                            self.update_signal.emit(f"Sending batch {batch_index + 1}/{total_batches} to {hostname}...")
                            ssh_shell.send(command_batch)
                            ssh_shell.send("\r")
                            QThread.msleep(2000)

                            # **Ensure the session remains open until the last batch is fully executed**
                            if batch_index == total_batches - 1:  # Last batch
                                # self.update_signal.emit(f"Waiting for final response from {hostname}...")
                                # response = self.send_command_and_wait(ssh_shell, "\r", hostname_pattern, wait_time=120)
                                self.update_signal.emit(f"Waiting for final response from {hostname}...")
                                response = self.send_command_and_wait(ssh_shell, command_batch, hostname_pattern, wait_time=120)
                        ...
                        #     # **READ SSH RESPONSE & EMIT IT IN REAL-TIME**
                        #     response = self.read_shell_output(ssh_shell)
                        #     log.write(response + '\n')

                        # response = self.read_shell_output(ssh_shell)  # Read final response
                        # log.write(response + '\n')

                        # self.update_signal.emit(f"Configuration completed for {hostname}.")

                except Exception as e:
                    self.update_signal.emit(f"Error processing {hostname}: {str(e)}")
    ...
    #                         if "%" in line or "Invalid input detected" in line:
    ...
    #                 ssh_shell.send("copy running-config startup-config\r")
    #                 ssh_shell.send("\r")  # Confirm save
    #                 QThread.msleep(2000)

    #                 response = self.read_shell_output(ssh_shell)
    #                 log.write(response + '\n')

    #                 # Log additional errors if found
    ...
    #                 error_log.flush()
    #                 error_log.seek(0)
    #                 error_lines_count = len(error_log.readlines())

    #                 summary_msg = f"{hostname} - Errors encountered, see log ({error_file})" if error_lines_count > 0 else f"{hostname} - Config completed"
    ...
    #                         updated_lines.append(summary_msg)
