...
class SaveLocationDialog(QMainWindow):  #This class used for GUI design and button handler
    ...
    def initUI(self):
        ...
        mainLayout.addWidget(self.check_button, 0, 4)    # Row 0, Column 2
        mainLayout.addWidget(self.Switch_username, 0, 3)
        mainLayout.addWidget(self.Switch_username_label,0,2)
        ...
        mainLayout.addWidget(self.Switch_password_input, 1, 3)
        mainLayout.addWidget(self.get_config_button, 1, 4)  # Row 1, Column 2
        mainLayout.addWidget(self.get_reload_button, 1, 5)  # Row 1, Column 3
        mainLayout.addWidget(self.Switch_password_label, 1,2)
    ...
    #             # Create and start a thread for configuring this device
    ...
    #             # Wait for the thread to finish before continuing to the next device
    #             self.thread.wait()  # Use thread.wait() to block until the thread has finished execution
...
class DeviceConfigThread(QThread):   #this class is being use for push the config to the devices.
    ...
    def run(self):
        ssh_client = None  # Initialize ssh_client outside the try block
        try:
            self.update_signal.emit("Connecting to the Server...")
            ...
            for port, hostname in self.selected_ports:
                ...
                with paramiko.SSHClient() as ssh_client:
                    ...
                    if hostname in self.device_config_map:
                        ...
                        batch_size = 50  # Adjust based on your device's capability
                    ...
                    with paramiko.SSHClient() as ssh_client:
                        ...
                        while True:
                            if ssh_shell.recv_ready():
                                ...
                                response += data
                                last_read_time = time.time()
                            elif time.time() - last_read_time > 3:  # 5 seconds timeout after last read
                                break
                            else:
                                time.sleep(0.1) 
